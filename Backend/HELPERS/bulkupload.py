...
class BulkFileUploadSerializer(serializers.Serializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(self.model, self.file_field):
            raise ValueError(
                f"Model '{model_name}' does not have a field named '{self.file_field}'."
            )
        ...
        for file in files:
            if file.size > 5 * 1024 * 1024:
                raise serializers.ValidationError(
                    f"File '{file.name}' exceeds the size limit of 5MB."
                )
        return files
            ...
            return Response(
                {
                    ...
                    "file_ids": [
                        getattr(instance, "id", None) for instance in created_files
                    ],
                    "file_uuids": [
                        getattr(instance, "uuid", None)
                        for instance in created_files
