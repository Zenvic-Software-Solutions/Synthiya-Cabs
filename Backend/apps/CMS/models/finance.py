...
class Finance(BaseModel):
    vehicle = models.ForeignKey(
        Vehicle, on_delete=models.CASCADE, related_name="finances"
    )
    identity = models.CharField(max_length=MAX_CHAR_FIELD_LENGTH)
    ...
    def __str__(self):
        return f"{self.identity} - {self.vehicle.vehicle_no}"


class FinanceHistory(BaseModel):
    finance = models.ForeignKey(
        Finance, on_delete=models.CASCADE, related_name="history"
    )
    ...
    def __str__(self):
        return f"{self.finance.identity} - {self.due_month} - {self.amount}"
