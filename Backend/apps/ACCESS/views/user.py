...
from apps.ACCESS.serializers import UserSerializer,RegisterSerializer,StaffSerializer,DriverSerializer
...
class ForgetPasswordAPIView(AppAPIView):
    def post(self,request,*args,**kwargs):
        ...
        if not phone_number and not password:
            return self.send_error_response({"error":"Phone no and password is required"})
        try:
